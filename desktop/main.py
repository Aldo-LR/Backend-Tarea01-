...
mydb = mysql.connector.connect(
    host="b1bsbmalzc8b8ynobtum-mysql.services.clever-cloud.com",
    user="uhims5ergrswp3ts",
    password="Jdfqc0E0nwmkQFpCxQeW",
    database="b1bsbmalzc8b8ynobtum"
)
...
class App:
    ...
    def __init__(self, root):
        ...
        GLabel_942["text"] = "Nombre"
        GLabel_942.place(x=30,y=80,width=80,height=25)
        ...
        self.codigo = GLineEdit_534
        self.nombre = GLineEdit_230
        self.fechahora = GLineEdit_55
        
        self.fechahora.insert(0,datetime.now().strftime("%d-%m-%Y %H:%M:%S"))
        GLineEdit_55.configure(state="disabled")
        
        

    def GButton_974_command(self):
        sqlInsertar = "insert into asistencia(codigo,nombre,fechahora) values(%s,%s,%s)"
        parametros = (self.codigo.get(),self.nombre.get(),self.fechahora.get())
        print(parametros)
        self.ejecutarSql(sqlInsertar,parametros)
        messagebox.showinfo("Asistencia ", "Asistencia registrada con Exito")
